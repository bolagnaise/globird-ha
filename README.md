...
ZeroHero status is date-aware and follows Home Assistant's configured timezone. It reports `achieved` or `missed` only when the latest complete cost day is today. Before the 18:00-21:00 ZeroHero window has finished it reports `pending`, then reports `awaiting_result` after 21:00 until GloBird publishes complete cost data for today. It reports `unknown` before any usable complete cost day is available.
